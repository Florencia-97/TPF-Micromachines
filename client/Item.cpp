...
void Item::_renderItem(SDL_Rect &camera, SDL_Renderer *renderer, int frames) {
    int x = rand()%2;
    int y = rand()%2;
  texture->render_with_size(mBox.x + x - camera.x,
							...
							ITEM_HEIGHT,
							false);
  dust.play(renderer, frames, mBox.x + x - camera.x, mBox.y + y - camera.y);
}

void Item::render(SDL_Rect &camera, SDL_Renderer *renderer, int frames, int x, int y) {
    mBox.x = x;
    mBox.y = y;
    _renderItem(camera, renderer, frames);
}
